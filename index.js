(() => {
  ...
  /* 日历标签页里已经接了视图的段位。四个都接上了 ——
     以后再加段位，不在这里的点了只切高亮、不换内容。 */
  const CAL_TAB_VIEWS = ["month", "week", "three", "day"];
  ...
  /* ---- 时间轴视图（周 / 三日）尺寸 ----
     HOUR 是一小时对应的像素高度，同时也是横向小时线的间距；它会由 JS 内联写成
     CSS 变量 --tt-wv-hour 传给样式表，所以这里就是唯一来源，改这里就够了。
  ...
  module.exports = class TimeTrail extends Plugin {
    ...
    _openSettingsModal() {
      ...
      /* 设置整合：左侧栏只有一个「日志设置」分类，右侧把现有的四个分组
         （插入设置 / 视图外观 / 日历设置 / 控制设置）一起展示。 */
      const GROUPS = [
        { key: "log", label: "日志设置", icon: "iconEdit" },
      ];
      const modal = document.createElement("div");
    ...
    _buildTypeSettings() {
      ...
      const itemEl = (item, gIdx, iIdx) => {
        ...
        el.innerHTML = `
          <input type="color" class="tt-types__item-color" title="点击选择颜色" />
        ...
        colorInput.value = item.color || DEFAULT_TYPE_COLOR;
      ...
      const groupEl = (group, gIdx) => {
        ...
        el.innerHTML = `
          <div class="tt-types__group-head">
            ...
            <input type="color" class="tt-types__group-color" title="分组颜色" />
        ...
        colorInput.value = group.color || DEFAULT_TYPE_COLOR;
    ...
    /* 把一天里的记录换算成时间轴上的块，返回 HTML 字符串数组。
       时长取到「下一条记录」为止 —— 和 Dock 时间线视图算「持续」用的是同一套规则，
       同一个数据源在两处不会给出两套说法。当天最后一条没有下一条可参照，
       就只显示起始时间、给一个最小高度（那段时间本来就没有数据）。
       top / height 在 JS 里算好写进 style：高度还得反过来决定时间行怎么摆，
       这件事 CSS 做不到（矮块里的两行会被裁掉半行）。 */
    _buildCalendarTimelineEvents(records) {
      ...
      return parsed.map((x, i) => {
        ...
        const top = (x.range.start / 60) * WEEK_HOUR_H;
    ...
    _buildCalendarTimelineHtml(days, byDate, todayKey) {
      ...
      const cols = days
        .map((d) => {
          ...
          const events = this._buildCalendarTimelineEvents(byDate[dKey] || []).join("");
          return `<div class="north-caltab-wv-col" data-date="${dKey}">${events}</div>`;
        })
        .join("");

      /* --tt-wv-hour / --tt-wv-cols 从这里写进 CSS：小时高与列数都只有 JS 这一个来源，
         样式表里的列高、小时间隔、网格列数都读它们，不会两边写岔。 */
      return `<div class="north-caltab-weekview" style="--tt-wv-hour:${WEEK_HOUR_H}px;--tt-wv-cols:${days.length}">
    ...
    _paintCalendarTab(container) {
      ...
      if (isTimeline) {
        ...
        if (wv) wv.scrollTop = WEEK_INITIAL_HOUR * WEEK_HOUR_H;
        return;
      }
    ...
    _renderLifeLogDockList(listEl, records) {
      ...
      const showDuration = !!(this.data && this.data.lifelogShowDuration);
      let html = "";
      sortedDates.forEach((date) => {
        ...
        items.forEach((item, idx) => {
          ...
          const rangeStr = prevItem ? `${prevItem.time} - ${item.time}` : item.time;
          let durationStr = "";
          if (prevItem && showDuration) {
            const parseTime = (t) => {
              const m = String(t || "").match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?$/);
              return m ? +m[1] * 3600 + +m[2] * 60 + (+m[3] || 0) : null;
            };
            const a = parseTime(prevItem.time);
            const b = parseTime(item.time);
            if (a !== null && b !== null) {
              let sec = b - a;
              if (sec < 0) sec = 0;
              const h = Math.floor(sec / 3600);
              const mm = Math.floor((sec % 3600) / 60);
              durationStr = h > 0 ? `${h}时${mm}分` : `${mm}分`;
            }
          }
